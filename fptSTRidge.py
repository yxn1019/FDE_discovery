...
from STRidge import *
#%%
def fptSTRidge(alpha, beta, noise_level=0):
    ...
    num=0
    Net.load_state_dict(torch.load(f'model_save/pt_fde_1d5-200-{noise_level}/pt_fde_1d5-10000.pkl'))
    x=torch.arange(3, 12, 0.1)
    t=torch.arange(5, 13, 0.1)
    ...
    if alpha<1:
        ...
        t_gj, w_gj = sp.roots_jacobi(n, 0, 1-alpha) # G-J points, 0<alpha<1
    if beta >1:
    ...
    # HHxxxx = PINNstatic * Hxxxx
    # HHxxxx_n HHxxxx.data.numpy()
    HxHx = Hx * Hx
    HxHx_n = HxHx.data.numpy()
    HxHxx = Hx * Hxx
    HxHxx_n=HxHxx.data.numpy()
    HxHx = Hx * Hx
    HxHx_n=HxHx.data.numpy()
    HxHxxx = Hx * Hxxx
    HxHxxx_n=HxHxxx.data.numpy()
    ...
    Theta = np.hstack((Theta, HxHx_n))
    Theta = np.hstack((Theta, HxHxx_n))
    Theta = np.hstack((Theta, HxHxxx_n))
